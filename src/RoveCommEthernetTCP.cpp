...
bool RoveCommEthernetTCP::read(RoveCommPacket &dest) {
  ...
  for (uint8_t i = 0; i < MAX_CLIENTS; i++) {
    ...
    int bytesRead = client.read(rovecomm::_readBuf, ROVECOMM_PACKET_HEADER_SIZE);
    if (bytesRead < ROVECOMM_PACKET_HEADER_SIZE)
      return false;
    if (rovecomm::_readBuf[0] != ROVECOMM_VERSION) {
      dest.dataId = RC_ROVECOMM_INVALID_VERSION_DATA_ID;
      return false;
    }
    uint16_t dataCount = (rovecomm::_readBuf[3] << 8) | rovecomm::_readBuf[4];
    uint8_t dataType = rovecomm::_readBuf[5];
    ...
    uint8_t *copyPtr = rovecomm::_readBuf + ROVECOMM_PACKET_HEADER_SIZE;
    do {
    ...
    if (rovecomm::unpackPacket(dest, rovecomm::_readBuf)) return true;
  }
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void RoveCommEthernetTCP::_writeReliable(const rovecomm::data_type_t dataType, const uint16_t dataId, const uint16_t dataCount, const void *data) {
  ...
  size_t sendSize = rovecomm::packPacket(rovecomm::_writeBuf, dataId, dataCount, dataType, reinterpret_cast<const uint8_t*>(data));
  ...
  do {
    size_t bytesSent = _TCPServer.write(rovecomm::_writeBuf + totalBytesSent, bytesRemaining);
...
void RoveCommEthernetTCP::_writeToReliable(const rovecomm::data_type_t dataType, const uint16_t dataId, const uint16_t dataCount, const void *data, 
  ...
  size_t totalBytesSent = 0;
  do {
    size_t bytesSent = client.write(rovecomm::_writeBuf + totalBytesSent, bytesRemaining);
    if (bytesSent == 0 || millis() - startTimestamp >= ROVECOMM_PACKET_WRITE_TIMEOUT) return;
    totalBytesSent += bytesSent;
    bytesRemaining -= bytesSent;
  } while (bytesRemaining > 0);
